...
export function validateNestedWhere(
  ...
  traverse(whereObject, (context) => {
    ...
    if (isLeaf) {
      const withoutBlacklistedWords = meta.nodePath!.replace(
        blackListedWordsRegex,
        '.',
      );
      const withoutRepeatedDots = withoutBlacklistedWords.replace(
        repeatedDotsRegex,
        '.',
      );
      const withoutLeadingAndTrailingDots = withoutRepeatedDots.replace(
        trimDotsRegex,
        '',
      );
      const cleanedupString = withoutLeadingAndTrailingDots.replace(
        lastFragmentRegex,
        '',
      );

      const isAllowed =
        !cleanedupString || allowedJoinsSet.has(cleanedupString);
      if (!isAllowed) {
        throw new BadRequestException(
          `Join relation not allowed: ${cleanedupString}`,
        );
      }
    }
  });
 ...
 */
export function validateNestedOrderBy(
  orderByObjects: any[],
  allowedJoinsSet: Set<string>,
) {
  const lastFragmentRegex = /\.?[^.]+$/;
...
export function validateCrudQueryFull(fullCrudQuery: CrudQueryFull) {
  ...
  if (error) {
    throw new BadRequestException(
      `fullCrudQuery did not match schema: \n${error}`,
    );
  }
}
