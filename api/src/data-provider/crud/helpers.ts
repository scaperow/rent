  ...
  traverse(copy, (context) => {
    ...
    if (
      ...
      if (valueIsArray) {
        ...
        const toUpdate = compact(
          value.map((v, index) => {
            if (!!v[idPropertyName]) {
              const originalNestedProperty = getNestedProperty(
                currentPersistedObject,
                `${meta.nodePath}`,
              );

              const onp = originalNestedProperty.find(
                (ov) => ov[idPropertyName] === v[idPropertyName],
              );
                ...
                const diff = fromPairs(
                  differenceWith(
                    toPairs(v).filter(
                      ([k, v]) =>
                        !isObject(v) &&
                        !['createdAt', 'updatedAt', 'id'].includes(k),
                    ),
                    toPairs(onp).filter(
                      ([k, v]) =>
                        !isObject(v) && !['createdAt', 'updatedAt'].includes(k),
                    ),
                    isEqual,
                  ),
                );
      ...
      // TODO: Refactor, split into named subfunctions at least the main condition blocks
      const persistedValue = getNestedProperty(
        currentPersistedObject,
        `${meta.nodePath}`,
      );
...
function getIdsToDisconnect(
  ...
  const stillConnectedSet = new Set(
    stillConnected.map((v) => v[idPropertyName]),
  );
