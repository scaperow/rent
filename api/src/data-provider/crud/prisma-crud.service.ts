...
export class PrismaCrudService {
  ...
  constructor(args: CrudServiceOpts) {
    ...
    this.defaultJoins = this.setDefaultJoins(
      args.defaultJoins,
  ...
  protected setPaginationConfig(
    userConfig?: PaginationConfig
  ...
  protected getFullCrudOpts(
    controllerOpts: CrudMethodOpts
  ): Required<CrudMethodOpts> {
    return Object.assign({}, DEFAULT_CRUD_METHOD_OPTS, controllerOpts);
  }

  protected getRepo(opts: CrudMethodOpts) {
  ...
  public createMany(createDto: any[], opts: CrudMethodOpts) {
    const fullOpts = this.getFullCrudOpts(opts);
    const repo = this.getRepo(fullOpts);
  ...
  public async findMany(opts: CrudMethodOpts) {
    ...
    const parsedCrudQuery = this.prismaQueryBuilder.parseCrudQuery(
      fullOpts.crudQuery
    );
    const { findManyQuery, pagination } =
      this.prismaQueryBuilder.buildFindManyQuery(parsedCrudQuery);
    ...
    if (
      parsedCrudQuery.select.only?.length ||
      parsedCrudQuery.select.except?.length
    ) {
      for (let i = 0; i < matches.length; i++) {
        const match = matches[i];
  ...
  public async findOne(id: string | number, opts: CrudMethodOpts) {
    if (id === null || id === undefined) {
      throw new InternalServerErrorException(
        `findOne received invalid id ${id}`
      );
    }
    ...
    const findOneQuery = this.prismaQueryBuilder.buildFindOneQuery(
      parsedCrudQuery,
      id
    );
  ...
  public async update(
   ...
   ///Only support original DB field struct, not prisma data struct
  public async updateMany(updateDto: any, opts: CrudMethodOpts, where: any) {
    const fullOpts = this.getFullCrudOpts(opts);
    const repo = this.getRepo(fullOpts);
