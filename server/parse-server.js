...
const ParseManager = require("parse-dashboard");
...
const { SERVER_DOMAIN, MONGO_URI, APP_ID, APP_NAME, SERVER_PORT, MASTER_KEY, SMTP_USER, SMTP_PASSWORD, SMTP_HOST, MANAGER_USER, MANAGER_PASSWORD, REPLICA_SET, SMTP_PORT } = process.env;
const sslForce = false;
const url = sslForce ? `https://${SERVER_DOMAIN}` : `http://${SERVER_DOMAIN}:5000`;
const parseUrl = url + "/api";
...
const graphQLServer = new ParseGraphQLServer(server, {
  graphQLPath: "/graphql",
  playgroundPath: "/playground",
});

const manager = new ParseManager(
  {
    graphQLServerURL: "http://localhost:5000/graphql",
    apps: [
      {
        graphQLServerURL: "http://localhost:5000/graphql",
        serverURL: parseUrl,
