...
const { SSL_FORCE: sslForce, WECHAT_APPID: wechatAppId, WECHAT_APPSECRET: wechatAppSecret, APP_NAME, SERVER_PORT, COMPANY_NAME, COMPANY_SHORT_NAME, CLIENT_URL } = process.env;
...
// parser.graphQLServer.applyGraphQL(app);
// (Optional) Mounts the GraphQL Playground - do NOT use in Production
// parser.graphQLServer.applyPlayground(app);
  ...
  passport.initialize(
    {
      ...
      provider: "wechat",
      clients: [{ platform: "web", appId: wechatAppId, appSecret: wechatAppSecret }],
    },
    app
  );
} else {
...
if (fs.existsSync(lockFileName)) {
  ...
  migrator.database.connect().then(
    (result) => {
      migrator.up(result.db, result.client).then(
        (migrated) => {
